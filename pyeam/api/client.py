...
class ClientHandler(BaseHTTPRequestHandler):
    ...
    def _handle_request(self, method):
        from pyeam.core.app import Builder
        ...
        if path in ClientServer.routes.keys():
            try:
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.send_header('Access-Control-Allow-Origin', Builder._config.build.dev_url)
                self.end_headers()

                # Execute invoke command
                response_data = ClientServer.routes[path]['command'](*args_list)
...
class ClientServer:
    ...
    def _run(cls, address: str="localhost", port=7330):
        server = HTTPServer((address, port), ClientHandler)
        print(f'API running on http://{address}:{port}')
